...
- **5.3 Map:** SwiftUI `Map`, annotations for filtered sites, clustering when dense. Pin fill by tier (Major=Gold, Notable=Terracotta, Minor=Stone-400), Stone-950 stroke; visited adds Jade check badge; selected pin enlarged w/ Gold-300 stroke + shadow; cluster badges Stone-950 bg / Gold-400 text. Tap pin → popover → `SiteDetailView` in `.sheet` `[.medium, .large]`. Filter change animates camera to fit visible annotations; zero → "No sites match" overlay.
